...
void RobotMap::init() {
    ...
    ((std::shared_ptr<CANSpeedController>)driverDrive1)->SetPID(1,0,0);
    ((std::shared_ptr<CANSpeedController>)drivelDrive1)->SetPID(1,0,0);
...
//    scalingscaleEnc->SetPIDSourceType(PIDSourceType::kDisplacement);
    ...
    scalingscaleLimit.reset(new DigitalInput(10));
    ...
    ((std::shared_ptr<CANSpeedController>)scalingscaleLift)->SetPID(1,0,0);

    driveshiftGear.reset(new Solenoid(0));

    //driveahrs.reset(new AHRS(SPI::kMXP));//TODO: Change this depending on the port
